...
from forward_locomotion_roboclip.go1_gym import MINI_GYM_ROOT_DIR
from forward_locomotion_roboclip.go1_gym.envs.base.base_task import BaseTask
from forward_locomotion_roboclip.go1_gym.utils.math_utils import quat_apply_yaw, wrap_to_pi, get_scale_shift
from forward_locomotion_roboclip.go1_gym.utils.terrain import Terrain
from forward_locomotion_roboclip.go1_gym.envs.base.legged_robot_config import Cfg
...
class LeggedRobot(BaseTask):
    ...
    def _prepare_reward_function(self):
        ...
        from forward_locomotion_roboclip.go1_gym.rewards.original_reward import OriginalReward
        from forward_locomotion_roboclip.go1_gym.rewards.eureka_reward import EurekaReward
    ...
    def _create_envs(self):
        ...
        if self.cfg.env.record_video:
            ...
            self.camera_props.height = 368
            # self.rendering_camera = self.gym.create_camera_sensor(self.envs[0], self.camera_props)
            # self.gym.set_camera_location(self.rendering_camera, self.envs[0], gymapi.Vec3(1.5, 1, 3.0),
            #                              gymapi.Vec3(0, 0, 0))

            self.rendering_camera_all_envs = []
            for env_id in range(len(self.envs)):
                camera = self.gym.create_camera_sensor(self.envs[env_id], self.camera_props)
                self.gym.set_camera_location(camera, self.envs[env_id], gymapi.Vec3(1.5, 1, 3.0),
                                          gymapi.Vec3(0, 0, 0))
                self.rendering_camera_all_envs.append(camera)
        ...
        self.video_frames_all_envs = [[] for _ in range(len(self.envs))]
        self.complete_video_frames_all_envs = [[] for _ in range(len(self.envs))]
    ...
    def log_video(self):
        ...
        #if it - self.last_recording_it >= RunnerArgs.save_video_interval or it == 0:
        ...
        frames_all_env = self.get_complete_frames_all_env()
        for env_id in range(len(frames_all_env)):
            frames = frames_all_env[env_id]
            if len(frames) > 0:
                self.pause_recording_for_env(env_id)
                #print("LOGGING VIDEO FOR ENV: ",env_id)
                logger.save_video(frames, f"videos/{env_id}/new.mp4", fps=1 / self.dt)
    ...
    def _render_headless(self):
        for env_id in range(len(self.envs)):
            if self.record_now and len(self.complete_video_frames_all_envs[env_id]) == 0:

                bx, by, bz = self.root_states[env_id, 0], self.root_states[env_id, 1], self.root_states[env_id, 2]
                self.gym.set_camera_location(self.rendering_camera_all_envs[env_id], self.envs[env_id], gymapi.Vec3(bx, by - 1.0, bz + 1.0),
                                            gymapi.Vec3(bx, by, bz))
                crt_video_frame = self.gym.get_camera_image(self.sim, self.envs[env_id], self.rendering_camera_all_envs[env_id],gymapi.IMAGE_COLOR)
                crt_video_frame = crt_video_frame.reshape((self.camera_props.height, self.camera_props.width, 4))
                self.video_frames_all_envs[env_id].append(crt_video_frame)

        # print("Number of Frames Saved: ",[len(f) for f in self.video_frames_all_envs], "  Reset buffer: ",self.reset_buf)

        for env_id in range(len(self.envs)):
            if self.reset_buf[env_id]:
                self.complete_video_frames_all_envs[env_id] = self.video_frames_all_envs[env_id][:]
                self.video_frames_all_envs[env_id] = []
    ...
    def pause_recording_for_env(self,env_id):
        self.complete_video_frames_all_envs[env_id] = []
        self.video_frames_all_envs[env_id] = []
    ...
    def get_complete_frames_all_env(self):

        complete_video_frames_all_envs = [vid if vid != None else [] for vid in self.complete_video_frames_all_envs]

        return complete_video_frames_all_envs 
